...
# ================= 界面设计部分 =================
# 稍微放大初始窗口，提供舒适的默认视觉
root = tb.Window(title="微课语音生成专业版 (多平台/版权所有)", themename="litera", size=(850, 700))
root.minsize(700, 580)

saved_config = load_config()

# ---- 1. 顶部 API 配置区 ----
api_frame = tb.Labelframe(root, text=" ⚙️ Azure 接口配置 (自动保存) ", padding=5, bootstyle=INFO)
...
entry_key = tb.Entry(api_frame, show="*") # 删除了 width=40，改为弹性拉伸
entry_key.grid(row=0, column=1, padx=5, pady=5, sticky="ew")
entry_key.insert(0, saved_config.get("speech_key", ""))

tb.Label(api_frame, text="区域 (Region):", font=("微软雅黑", 10)).grid(row=0, column=2, padx=(10, 5), pady=5, sticky="e")
...
voice_combo.grid(row=0, column=1, columnspan=5, padx=5, pady=5, sticky="ew")
voice_combo.current(0)

tb.Label(top_frame, text="语速:").grid(row=1, column=0, sticky="e", pady=5)
rate_scale = tb.Scale(top_frame, from_=-50, to=50, orient=tk.HORIZONTAL, bootstyle=INFO) # 删除了固定的 length 限制
rate_scale.set(0)
rate_scale.grid(row=1, column=1, padx=5, sticky="ew")

tb.Label(top_frame, text="音调:").grid(row=1, column=2, sticky="e")
pitch_scale = tb.Scale(top_frame, from_=-50, to=50, orient=tk.HORIZONTAL, bootstyle=WARNING)
pitch_scale.set(0)
pitch_scale.grid(row=1, column=3, padx=5, sticky="ew")

tb.Label(top_frame, text="音量:").grid(row=1, column=4, sticky="e")
volume_scale = tb.Scale(top_frame, from_=0, to=100, orient=tk.HORIZONTAL, bootstyle=SUCCESS)
volume_scale.set(100)
volume_scale.grid(row=1, column=5, padx=5, sticky="ew")

btn_reset = tb.Button(top_frame, text="↺ 重置", command=reset_params, bootstyle=(SECONDARY, OUTLINE))
btn_reset.grid(row=1, column=6, padx=(10, 0), sticky="e")
...
# 关键修复：将文本框的初始 height 调小，避免强制撑开窗口导致底部被挤掉。依然会自动伸缩填满剩余空间。
text_input = tk.Text(text_frame, height=4, font=("微软雅黑", 11), wrap=tk.WORD, undo=True, maxundo=-1, relief=tk.FLAT, bg="#F8F9FA")
...
export_frame.pack(side=tk.TOP, pady=5)

convert_btn_mp3 = tb.Button(export_frame, text="🎵 导出 MP3", command=lambda: on_convert("mp3"), bootstyle=SUCCESS, width=18)
convert_btn_mp3.pack(side=tk.LEFT, padx=10)

convert_btn_wav = tb.Button(export_frame, text="🎚️ 导出 WAV (无损)", command=lambda: on_convert("wav"), bootstyle=PRIMARY, width=18)
convert_btn_wav.pack(side=tk.LEFT, padx=10)
...
author_label.pack(side=tk.BOTTOM, pady=(0, 2))
